...
<div class="container w-full h-full grow p-4">
	...
	<div class="mobile-nav w-full flex justify-around border-b-2 mb-4">
		...
		<a
			class={`text-lg font-bold p-2 rounded-md ${
				data.url.includes('/admin/products') ? 'bg-neutral-100' : ''
			}`}
			href="/admin/products">Products</a
		>
		<a
			class={`text-lg font-bold p-2 rounded-md ${
				data.url.includes('/admin/orders') ? 'bg-neutral-100' : ''
			}`}
	...
	<div
		class="desktop-nav bg-neutral-200 shadow-xl w-1/6 rounded-md p-4 flex flex-col gap-y-3 items-start mr-4"
	>
		<a
